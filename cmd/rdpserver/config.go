...
func parseFlags() *config {
	...
	flag.BoolVar(&cfg.allowPasswordless, "allow-passwordless", getEnvBool("ALLOW_PASSWORDLESS", false), "Deprecated compatibility flag; empty passwords are rejected in per-user login mode")
	...
	flag.IntVar(&cfg.maxConnsPerIP, "max-conns-per-ip", getEnvNonNegativeInt("MAX_CONNS_PER_IP", 3), "Maximum concurrent WebSocket connections per source IP (0 disables the per-IP limit)")
